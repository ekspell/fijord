...
const tabs = ["Discovery", "Scope", "Staging", "Brief"];

export default function TopNav() {
  const { activeTab, setActiveTab, result, roadmap, setShowLanding, linearApiKey, clearLinearApiKey, jiraCreds, clearJiraCreds, firefliesApiKey, clearFirefliesApiKey } = useNav();
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-border bg-background/85 px-8 backdrop-blur-md" style={{ height: 56 }}>
      {/* Logo */}
      <button className="flex items-center" onClick={() => { setShowLanding(true); setActiveTab("Discovery"); }}>
